...
Das Starburst Projekt \cite{lohman1988Starbust}, \cite{haas1989extensible}, vorangetrieben und entwickelt von IBM,  startet unter der Prämisse, dass bestehende DBMSe nicht in der Lage sind den wachsenden Anforderungen von verschiedenen, neuartigen Applikationen vollumfänglich zu entsprechen. Zur Erfüllung der individuellen Ansprüche wurde das Starburst Projekt begonnen. Sein Ziel ist es dem \ac{DBI} die Möglichkeit zu geben, die relationale Datenbank zu erweitern und so die Bedürfnisse von spezifischen Anwendungen zu erfüllen. Beispielsweise ist es möglich, neue Zugriffs- und Speichermethoden zu implementieren oder neue Join Methoden zu erstellen. Um diese Features zu ermöglichen, stellt Starbust eine Anfragesprache, einen regelbasierten Optimierer, Query Rewriter und ein Ausführungssystem basierend auf relationaler Algebra zur Verfügung.

Ähnlich wie bei anderen Systemen wird zwischen zwei Systemen bei der Ausführung unterschieden. Der genaue Ablauf ist nochmals im Folgenden beschrieben. Daraufhin wird auf die Starburst eigene Regelmaschine eingegangen, die für das Ausführen von Regeln zur Anfragenoptimierung gedacht ist.
...
Zur Comile-Time wird eine gegebene Anfrage auf semantische Korrektheit geprüft und in ein \ac{QGM} übersetzt. Basierend auf diesem \ac{QGM} wird die Optimierung der Anfrage zuerst durch einen Query Rewriter, einen Plan Optimierer und einen Plan Refiner durchgeführt.
...
\item Bei der Nutzung eines QGMs kann entweder eine Tiefen- als auch eine Breitensuche zur Anwendung kommen. Diese Art der Suche ist weder an die Regel noch an das Regel-Set geknüpft, sondern mit dem QGM selbst verbunden.
...
\subsubsection{Starburst Plan Generator}
...
\item Das Ausführen von Regeln wird effizienter, da nicht mehr ganze Graphen nach Pattern durchsucht werden müssen, sondern direkt über Build Blocks erkannt werden. Dadurch erleichtert sich die Ausführung. Ebenfalls ist es dank der Nutzung von Macro-Expandern möglich, die Geschwindigkeit zu verbessern.
