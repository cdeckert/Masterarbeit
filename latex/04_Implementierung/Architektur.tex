...
Nicht alle Komponenten wurden selbst entwickelt, beispielsweise wurde beim Logging auf die existierende Library easylogging++ gesetzt. Auch für das Parsen von JSON-Files ist die externe Bibliothek json11 verwendet worden. 
...
\end{figure}]\todo{Grafik überarbeiten (nicht mehr 90Grad)}
...
\textit{Commutativity}, \textit{LeftAssociativity}, \textit{RighAssociativity} und \textit{Swap} bilden die erste Ebene der Regeln. Sie kommen in Verbindung mit den Regelmengen \textit{RS-B0} und \textit{RS-B1} zum Einsatz.

Auf dem zweiten Level, diese Klassen sind mit dem Zusatz \textit{B2} gekennzeichnet, finden sich \textit{Commutativity-B2}, \textit{LeftAssociativity-B2}, \textit{RighAssociativity-B2} und \textit{Exchange-B2}. Diese Regeln sind für das Regelset \textit{RS-B2} notwendig und erlauben es andere Regeln zu deaktivieren. Da die Deaktivierung von Regeln eine Erweiterung darstellt, erben Kommutativität und Assoziativität von ihren Verwandten aus \textit{RS-B0} bzw. \textit{RS-B1}. Da die Exchange Regel neu hinzukommt, erbt diese direkt von Rule.

Auf dem dritten Level findet sich die \textit{GraphRule}. Sie ersetzt andere Regeln und ist in der Regelmenge \textit{GraphRuleSet} abgebildet.


Ähnlich wie bei Starburst bestehen Regel aus zwei Teilen. Auf der einen Seite wird geprüft, ob eine Regel zur Anwendung kommen kann. Dies geschieht mit der Methode \texttt{isApplicable} auf der anderen Seite gibt es die Methode \texttt{apply}, die immer einen äquivalenten Planknoten zurückliefert. Mit Hilfe dieses Prinzips wurden alle Regeln implementiert. Das einheitliche Interface erlaubt es, dass Regeln in Regelsets gebündelt werden können.


Die konkreten Bedingungen für die Anwendbarkeit einer Regel sind in Tabelle \ref{TabelleRules} abzulesen. Die Anwendbarkeit ist je nach Regel von den Nachbarn eines Knoten abhängig. Das Konzept der Nachbarn und der referenzierten Relationen ist in \ref{sec:PlanNode} beschrieben. 
...
\begin{tabular}{@{}llll@{}}
\toprule
{\bf Regel}   & {\bf Eingabe} & {\bf Bedingung}      & {\bf Ausgabe} \\ \midrule
Commutativity & $A \Join B$ & $op = \Join \&\& A.isOverlapping(B)$ & B \Join A    \\
              &             &                      &              \\
              &             &                      &              \\ \bottomrule
\end{tabular}
\label{TabelleRules}
\caption{Regelübersicht}
\end{table}
