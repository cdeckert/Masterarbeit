...
#ifndef __RuleBasedOptimizer__Reservoir__
#define __RuleBasedOptimizer__Reservoir__
...
public:
	...
	{
		return &(_chunks[idx_chunk(i)][idx_in_chunk(i)]);
	}
	inline       T *operator[](const uint i)
	{
		return &(_chunks[idx_chunk(i)][idx_in_chunk(i)]);
	}
	inline uint cardinality() const
	{
		return _cardinality;
	}
	inline uint noChunks() const
	{
		return _chunks.size();
	}
public:
...
public:
	inline T *first_of_chunk(uint aChunkNo)
	{
		return (&(_chunks[aChunkNo][0]));
	}
	inline T *end_of_chunk(uint aChunkNo);
private:
	T *allocChunk();
	inline uint idx_chunk(uint i) const
	{
		return (i >> _log2chunksize);
	}
	inline uint idx_in_chunk(uint i) const
	{
		return (i & _mask);
	}
private:
...
#endif /* defined(__RuleBasedOptimizer__Reservoir__) */
